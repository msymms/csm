...
if (!function_exists('scsdesigns_csm_setup')) :
    ...
    function scsdesigns_csm_setup() {
        ...
        /*
         *  Edding multiple images sizes for thumbnails to allow for 
         *  srcset 
         */

        add_image_size('thumbnail_300w', 300, 200);
        add_image_size('thumbnail_360w', 360, 240);
        add_image_size('thumbnail_480w', 480, 320);
        add_image_size('thumbnail_600w', 600, 400);
...
function scsdesigns_csm_scripts() {
    ...
    wp_enqueue_script('scsdesigns-csm-fixed-menu', get_template_directory_uri() . '/js/fixed-menu.js', array('jquery'), '20180407', true);
...
function csm_content_image_sizes_attr($sizes, $size) {
    ...
    if (740 <= $width) {
        $sizes = '(max-width: 706px) 89vw, (max-width: 767px) 90vw, 1400px';
    }
...
function csm_post_thumbnail_sizes_attr($attr, $attachment, $size) {
    if (is_archive() || is_search() || is_home()) {
        $attr['sizes'] = '(max-width: 600px) 89vw, (max-width: 1000px) 54vw, (max-width: 1071px) 75vw, 75vw';
    } else {
        $attr['sizes'] = '100vw';
    }
